...
function resolveContextualGlosarium(raw, data) {
  let list = [];
  const d = data || {};
  const topik = (d.topikMateri || d.isiTopikMateri || 'Materi Pokok').trim();
  const mapel = (d.mataPelajaran || 'Mata Pelajaran').trim();
  ...
  // 1. Jika masih kurang dari 6, gali istilah dari materiAjarDeskriptif
  if (list.length < 6 && d && d.materiAjarDeskriptif && typeof d.materiAjarDeskriptif === 'string') {
    const text = d.materiAjarDeskriptif;
    const regexPatterns = [
      /(?:^|\n)(?:[a-z\d]+[.)]|\*|-)?\s*\*\*([A-Za-z0-9\s–\(\)\/-]{3,45})\*\*\s*[:–-]?\s*([^.\n]+(?:\.[^.\n]+)?)/gi,
      /(?:^|\n)#{1,4}\s+([A-Za-z0-9\s–\(\)\/-]{3,45})/gi
    ];
    for (const rgx of regexPatterns) {
      let m;
      while ((m = rgx.exec(text)) !== null && list.length < 6) {
        const term = (m[1] || '').trim();
        const def = (m[2] || '').trim() || `Konsep kunci dan kaidah penting dalam materi ${topik}.`;
        if (term.length > 2 && term.length < 50 && !forbiddenTerms.some(fb => term.toLowerCase().includes(fb))) {
          if (!list.some(existing => existing.istilah.toLowerCase() === term.toLowerCase())) {
  ...
  // 2. Jika masih kurang dari 6, sertakan istilah keilmuan kontekstual murni materi berdasarkan topik & mata pelajaran
  if (list.length < 6) {
    const fullCtx = `${topik} ${mapel} ${d.jurusanSekolah || ''} ${d.materiTambahan || ''}`.toLowerCase();
    let domainTerms = [];

    // Konteks Biologi / Evolusi / Alam (termasuk Teori Seleksi Alam)
    if (/\b(seleksi\s*alam|evolusi|biologi|spesiasi|adaptasi|genetik|ekosistem|organisme|hayati|sel|dna|mutasi|darwin)\b/i.test(fullCtx)) {
      domainTerms = [
        { istilah: "Seleksi Alam (Natural Selection)", definisi: "Mekanisme evolusi di mana individu dengan variasi sifat fenotip yang lebih adaptif terhadap lingkungan memiliki peluang bertahan hidup dan bereproduksi lebih tinggi untuk mewariskan sifatnya." },
        { istilah: "Variasi Genetik (Genetic Variation)", definisi: "Keanekaragaman susunan gen atau alel antar-individu dalam suatu populasi yang timbul dari mutasi spontan, rekombinasi seksual, dan aliran gen sebagai materi dasar seleksi alam." },
        { istilah: "Adaptasi (Adaptation)", definisi: "Penyesuaian karakteristik morfologis, fisiologis, atau perilaku organisme yang berkembang melalui seleksi bertahap guna meningkatkan kebugaran hidup di habitat alaminya." },
        { istilah: "Kebugaran Evolusioner (Fitness)", definisi: "Derajat keberhasilan relatif suatu individu atau genotipe dalam bertahan hidup, bereproduksi, dan mewariskan salinan gennya ke generasi penerus." },
        { istilah: "Tekanan Seleksi (Selection Pressure)", definisi: "Faktor lingkungan biotik (pemangsa, kompetitor, patogen) maupun abiotik (iklim, ketersediaan air, suhu) yang membatasi kesintasan individu tertentu." },
        { istilah: "Spesiasi (Speciation)", definisi: "Proses evolusioner terbentuknya satu atau lebih spesies baru yang terisolasi secara reproduktif dari populasi leluhurnya." },
        { istilah: "Homologi & Analogi Anatomi", definisi: "Homologi merujuk pada kemiripan struktur tubuh akibat asal-usul filogenetik yang sama, sedangkan analogi adalah kemiripan fungsi organ yang berkembang secara independen (evolusi konvergen)." }
      ];
    } else if (/\b(videografi|sinematografi|tata\s*kamera|kamera\s*video|film|broadcasting)\b/i.test(fullCtx)) {
      domainTerms = [
        { istilah: "Shot Size (Ukuran Bidik)", definisi: "Klasifikasi luas area pandang kamera terhadap subjek dan latar, mulai dari Extreme Long Shot, Medium Shot, Close-Up, hingga Extreme Close-Up." },
        { istilah: "Camera Angle (Sudut Pandang)", definisi: "Elevasi sumbu vertikal kamera terhadap subjek, meliputi Bird Eye View, High Angle, Eye Level, Low Angle, dan Dutch Angle." },
        { istilah: "Camera Movement (Pergerakan Kamera)", definisi: "Dinamika pergerakan kamera untuk membangun ritme visual, mencakup Pan, Tilt, Dolly/Track, Pedestal, dan Zoom." },
        { istilah: "Depth of Field (DoF)", definisi: "Rentang zona ketajaman fokus di depan dan di belakang subjek yang dipengaruhi aperture, panjang fokus lensa, dan jarak bidik." },
        { istilah: "Rule of Thirds & Framing", definisi: "Kaidah pembagian bidang bidik menjadi kisi proporsional 3x3 untuk menempatkan titik minat visual dan keseimbangan komposisi gambar." },
        { istilah: "White Balance (WB)", definisi: "Kalibrasi sensor kamera untuk menyesuaikan suhu warna pencahayaan (derajat Kelvin) agar warna objek putih terekam netral." }
      ];
    } else if (/\b(tipografi|typography|typeface|font|huruf|dkv|desain\s*grafis)\b/i.test(fullCtx)) {
      domainTerms = [
        { istilah: "Typeface vs Font", definisi: "Typeface adalah rancangan rumpun keluarga visual huruf (misal: Helvetica), sedangkan Font adalah berkas digital spesifik pada gaya dan ukuran tertentu." },
        { istilah: "Kerning & Tracking", definisi: "Kerning adalah pengaturan jarak horizontal antar-dua karakter huruf spesifik, sedangkan Tracking adalah jarak spasi seragam antar-seluruh karakter teks." },
        { istilah: "Leading (Line Spacing)", definisi: "Jarak vertikal antara baseline satu baris teks ke baseline baris teks berikutnya untuk kenyamanan ritme membaca." },
        { istilah: "Hierarki Tipografi", definisi: "Penataan kontras ukuran, bobot, dan tata letak teks untuk memandu alur mata audiens membaca informasi secara terstruktur." },
        { istilah: "Legibility & Readability", definisi: "Legibility adalah kemudahan mengenali bentuk fisik tiap karakter huruf, sedangkan Readability adalah kenyamanan membaca teks panjang." },
        { istilah: "Grid System", definisi: "Struktur garis kisi panduan kolom dan margin untuk menciptakan keselarasan proporsi layout desain." }
      ];
    } else if (/\b(fisika|mekanika|energi|gaya|kinematika|dinamika|listrik|magnet|gelombang)\b/i.test(fullCtx)) {
      domainTerms = [
        { istilah: "Besaran Vektor & Skalar", definisi: "Besaran fisika yang memiliki nilai dan arah tertentu (vektor) dibandingkan dengan besaran yang hanya memiliki nilai numerik saja (skalar)." },
        { istilah: "Hukum Kekekalan Energi", definisi: "Prinsip fundamental fisika bahwa total energi dalam sistem terisolasi bersifat konstan dan hanya dapat berubah bentuk." },
        { istilah: "Dinamika Gaya & Momentum", definisi: "Kajian interaksi gaya yang bekerja pada suatu benda sehingga menimbulkan percepatan dan perubahan momentum sesuai Hukum Newton." },
        { istilah: "Parameter Pengukuran & Ketidakpastian", definisi: "Derajat ketelitian dan presisi instrumen alat ukur fisis serta batas toleransi kesalahan eksperimental." },
        { istilah: "Efisiensi Energi Sistem", definisi: "Perbandingan matematis antara energi berguna yang dihasilkan dengan total energi masukan yang diserap sistem." }
      ];
    } else if (/\b(kimia|larutan|stoikiometri|reaksi|molekul|senyawa|asam|basa)\b/i.test(fullCtx)) {
      domainTerms = [
        { istilah: "Stoikiometri Reaksi", definisi: "Hubungan kuantitatif perbandingan mol, massa, dan volume antara reaktan dan produk dalam suatu persamaan reaksi kimia berimbang." },
        { istilah: "Ikatan Kimia", definisi: "Gaya tarik-menarik elektrostatik antaratom atau antarmolekul yang membentuk struktur senyawa stabil (kovalen, ionik, logam)." },
        { istilah: "Kesetimbangan Kimia Dinamis", definisi: "Kondisi di mana laju reaksi maju sama dengan laju reaksi balik sehingga konsentrasi zat-zat dalam sistem tetap konstan." },
        { istilah: "Energi Aktivasi & Laju Reaksi", definisi: "Energi minimum yang diperlukan partikel reaktan untuk bereaksi, yang dipengaruhi konsentrasi, luas permukaan, suhu, dan katalis." }
      ];
    } else if (/\b(matematika|aljabar|kalkulus|geometri|trigonometri|statistik)\b/i.test(fullCtx)) {
      domainTerms = [
        { istilah: "Relasi & Domain Fungsi", definisi: "Himpunan daerah asal yang memetakan setiap elemen nilai masukan tepat satu ke daerah hasil (kodomain)." },
        { istilah: "Teorema & Pembuktian Deduktif", definisi: "Pernyataan matematis sahih yang dibuktikan kebenarannya melalui aksioma dan penalaran logika berantai." },
        { istilah: "Regresi & Korelasi Analitis", definisi: "Model pengukuran matematis untuk menaksir kekuatan hubungan dan arah dependensi antar-dua variabel data atau lebih." },
        { istilah: "Optimasi Fungsi Nilai Ekstrem", definisi: "Proses menemukan nilai maksimum atau minimum suatu fungsi objektif dengan mempertimbangkan batasan kendala yang ada." }
      ];
    } else if (/\b(informatika|komputer|coding|pemrograman|software|jaringan|database|algoritma)\b/i.test(fullCtx)) {
      domainTerms = [
        { istilah: "Algoritma & Kompleksitas Komputasi", definisi: "Rangkaian instruksi logis terurut untuk menyelesaikan masalah komputasi dengan analisis efisiensi waktu dan memori." },
        { istilah: "Struktur Data (Data Structure)", definisi: "Format organisasi dan penyimpanan data di dalam memori komputer agar proses manipulasi dan penelusuran berlangsung efisien." },
        { istilah: "Protokol Komunikasi Jaringan", definisi: "Standar aturan baku pertukaran format data dan penanganan kesalahan transmisi antarperangkat pada jaringan komputer." },
        { istilah: "Integritas & Enkripsi Data", definisi: "Mekanisme pengamanan informasi digital melalui algoritma penyandian kriptografis guna mencegah modifikasi tidak sah." }
      ];
    } else {
      domainTerms = [
        { istilah: `Konsep Esensial ${topik}`, definisi: `Prinsip fundamental, batasan keilmuan, dan teori dasar yang melandasi pemahaman materi ${topik} dalam mata pelajaran ${mapel}.` },
        { istilah: `Mekanisme & Prinsip Kerja ${topik}`, definisi: `Kaidah operasional, relasi fungsional, dan alur kerja sistematis yang menjadi proses inti materi ${topik}.` },
        { istilah: `Parameter Kunci ${topik}`, definisi: `Variabel terukur, spesifikasi standar, dan tolok ukur kualitas yang menentukan keberhasilan penerapan materi ${topik}.` },
        { istilah: `Aplikasi Terapan ${topik}`, definisi: `Penerapan konkret konsep dan keterampilan materi ${topik} dalam konteks pemecahan masalah dunia nyata dan dunia industri.` },
        { istilah: `Analisis Prosedur ${topik}`, definisi: `Langkah kerja analitis terstruktur dan pedoman teknis yang baku dalam mendalami materi ${topik}.` },
        { istilah: `Standar Baku Mutu ${topik}`, definisi: `Kriteria evaluasi dan tolok ukur kelayakan hasil kerja yang berlaku profesional pada bidang materi ${topik}.` }
      ];
    }

    domainTerms.forEach(item => {
      if (list.length < 6 && !list.some(existing => existing.istilah.toLowerCase() === item.istilah.toLowerCase())) {
        list.push(item);
      }
    });
  }
...
function resolveContextualDaftarPustaka(raw, data) {
  ...
  const topik = (d.topikMateri || d.isiTopikMateri || 'Materi Pokok').trim();
  const mapel = (d.mataPelajaran || 'Mata Pelajaran').trim();
  const jenjang = (d.jenjangSekolah || 'SMA/MA').trim();
  ...
  // Jika referensi dari AI kurang dari 4, sertakan literatur buku teks ilmiah otoritatif materi yang relevan
  if (list.length < 4) {
    const fullCtx = `${topik} ${mapel} ${d.jurusanSekolah || ''} ${d.materiTambahan || ''}`.toLowerCase();
    let domainPustaka = [];

    // Konteks Biologi / Evolusi / Alam (termasuk Teori Seleksi Alam)
    if (/\b(seleksi\s*alam|evolusi|biologi|spesiasi|adaptasi|genetik|ekosistem|darwin)\b/i.test(fullCtx)) {
      domainPustaka = [
        `Campbell, N. A., Reece, J. B., Urry, L. A., Cain, M. L., Wasserman, S. A., Minorsky, P. V., & Jackson, R. B. (2020). <em>Biologi Edisi Kedelapan (Jilid 1 & 2)</em>. Jakarta: Penerbit Erlangga.`,
        `Darwin, C. (1859). <em>On the Origin of Species by Means of Natural Selection, or the Preservation of Favoured Races in the Struggle for Life</em>. London: John Murray.`,
        `Futuyma, D. J., & Kirkpatrick, M. (2017). <em>Evolutionary Biology</em> (4th ed.). Sunderland: Sinauer Associates / Oxford University Press.`,
        `Kementerian Pendidikan, Kebudayaan, Riset, dan Teknologi. (2022). <em>Buku Teks Utama Biologi untuk SMA/MA Kelas XI & XII</em>. Jakarta: Pusat Perbukuan Kemendikbudristek.`,
        `Freeman, S., & Herron, J. C. (2014). <em>Evolutionary Analysis</em> (5th ed.). Boston: Pearson Education.`
      ];
    } else if (/\b(videografi|sinematografi|tata\s*kamera|kamera\s*video|film|broadcasting)\b/i.test(fullCtx)) {
      domainPustaka = [
        `Brown, B. (2021). <em>Cinematography: Theory and Practice: For Cinematographers and Directors</em> (4th ed.). New York: Routledge.`,
        `Mascelli, J. V. (2005). <em>The Five C's of Cinematography: Motion Picture Filming Techniques</em>. Los Angeles: Silman-James Press.`,
        `Thompson, R., & Bowen, C. J. (2017). <em>Grammar of the Shot</em> (4th ed.). New York: Focal Press.`,
        `Zettl, H. (2016). <em>Sight, Sound, Motion: Applied Media Aesthetics</em> (8th ed.). Boston: Cengage Learning.`
      ];
    } else if (/\b(tipografi|typography|typeface|font|huruf|dkv|desain\s*grafis)\b/i.test(fullCtx)) {
      domainPustaka = [
        `Lupton, E. (2014). <em>Thinking with Type: A Critical Guide for Designers, Writers, Editors, & Students</em> (2nd ed.). New York: Princeton Architectural Press.`,
        `Bringhurst, R. (2012). <em>The Elements of Typographic Style</em> (Version 4.0). Seattle: Hartley & Marks Publishers.`,
        `Ambrose, G., & Harris, P. (2017). <em>The Fundamentals of Typography</em> (2nd ed.). London: Bloomsbury Visual Arts.`,
        `Landa, R. (2018). <em>Graphic Design Solutions</em> (6th ed.). Boston: Cengage Learning.`
      ];
    } else if (/\b(fisika|mekanika|energi|gaya|listrik|magnet)\b/i.test(fullCtx)) {
      domainPustaka = [
        `Halliday, D., Resnick, R., & Walker, J. (2018). <em>Fundamentals of Physics</em> (11th ed.). New York: John Wiley & Sons.`,
        `Serway, R. A., & Jewett, J. W. (2018). <em>Physics for Scientists and Engineers</em> (10th ed.). Boston: Cengage Learning.`,
        `Tipler, P. A., & Mosca, G. (2015). <em>Fisika untuk Sains dan Teknik (Edisi Ketiga)</em>. Jakarta: Penerbit Erlangga.`,
        `Kementerian Pendidikan, Kebudayaan, Riset, dan Teknologi. (2022). <em>Buku Teks Siswa dan Panduan Guru Fisika SMA/MA</em>. Jakarta: Pusat Perbukuan.`
      ];
    } else if (/\b(kimia|larutan|stoikiometri|reaksi|senyawa)\b/i.test(fullCtx)) {
      domainPustaka = [
        `Chang, R., & Goldsby, K. A. (2016). <em>Chemistry</em> (12th ed.). New York: McGraw-Hill Education.`,
        `Brown, T. L., LeMay, H. E., Bursten, B. E., & Murphy, C. J. (2017). <em>Chemistry: The Central Science</em> (14th ed.). Boston: Pearson.`,
        `Petrucci, R. H., Herring, F. G., Madura, J. D., & Bissonnette, C. (2017). <em>Kimia Dasar: Prinsip-Prinsip dan Aplikasi Modern</em>. Jakarta: Penerbit Erlangga.`
      ];
    } else if (/\b(matematika|aljabar|kalkulus|geometri|statistik)\b/i.test(fullCtx)) {
      domainPustaka = [
        `Stewart, J. (2016). <em>Calculus: Early Transcendentals</em> (8th ed.). Boston: Cengage Learning.`,
        `Anton, H., Bivens, I., & Davis, S. (2016). <em>Calculus</em> (10th ed.). New York: John Wiley & Sons.`,
        `Walpole, R. E., Myers, R. H., & Ye, K. (2016). <em>Probability & Statistics for Engineers & Scientists</em> (9th ed.). Boston: Pearson.`
      ];
    } else if (/\b(informatika|komputer|coding|pemrograman|software|jaringan)\b/i.test(fullCtx)) {
      domainPustaka = [
        `Cormen, T. H., Leiserson, C. E., Rivest, R. L., & Stein, C. (2022). <em>Introduction to Algorithms</em> (4th ed.). Cambridge: MIT Press.`,
        `Tanenbaum, A. S., & Wetherall, D. J. (2021). <em>Computer Networks</em> (6th ed.). Boston: Pearson.`,
        `Pressman, R. S., & Maxim, B. R. (2020). <em>Software Engineering: A Practitioner's Approach</em> (9th ed.). New York: McGraw-Hill.`
      ];
    } else {
      domainPustaka = [
        `Tim Penulis Akademik. (${tahunAjar}). <em>Buku Teks ${mapel}: Telaah Teoretis dan Praktis Materi ${topik} untuk Peserta Didik ${jenjang}</em>. Jakarta: Penerbit Buku Pendidikan Nasional.`,
        `Kementerian Pendidikan, Kebudayaan, Riset, dan Teknologi. (${tahunAjar}). <em>Buku Panduan Guru dan Sumber Belajar Siswa Mata Pelajaran ${mapel}</em>. Jakarta: Pusat Kurikulum dan Perbukuan.`,
        `Sudjana, N. & Ibrahim. (2019). <em>Bahan Kajian Akademik dan Pendalaman Keilmuan ${mapel}</em>. Bandung: Sinar Baru Algensindo.`,
        `Asosiasi Pendidik dan Profesi ${mapel} Indonesia. (${tahunAjar - 1}). <em>Standar Keilmuan dan Praktik Terapan Materi ${topik}</em>. Jakarta: Konsorsium Keilmuan Terapan.`
      ];
    }

    domainPustaka.forEach(ref => {
      if (list.length < 5 && !list.some(existing => existing === ref)) {
        list.push(ref);
      }
    });
  }
